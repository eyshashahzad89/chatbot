...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['delete_one']) && isset($_POST['id'])) {
        delete_message_permanently((int) $_POST['id'], $email);
        header('Location: trash.php');
        exit;
    }
}

$trashed = get_trashed_messages($email);
?>
<!doctype html>
<html lang="en">
<head>
...
<div class="app" id="app">
    ...
    <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <a class="nav-item active" href="trash.php">
    ...
    <main class="main">
        ...
        <header class="main-header">
            <div class="header-left">
                ...
                <div>
                    <h1>Trash</h1>
                    <p>Restore or permanently remove chat messages.</p>
                </div>
            </div>
            ...
            <div class="header-actions">
                ...
                <form method="post" style="display:inline">
                    <button class="btn-ghost" type="submit" name="restore_all" value="1" <?= empty($trashed) ? 'disabled' : '' ?>>Restore all</button>
                </form>

                <form method="post" style="display:inline" onsubmit="return confirm('Permanently delete all trashed messages?');">
                    <button class="btn-danger" type="submit" name="empty_trash" value="1" <?= empty($trashed) ? 'disabled' : '' ?>>Empty trash</button>
                </form>
            </div>
        </header>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        <?php foreach ($trashed as $m): ?>
                            <tr>
                                <td><?= $m['sender'] === 'user' ? 'You' : 'Assistant' ?></td>
                                <td><?= htmlspecialchars($m['message']) ?></td>
                                <td class="mono"><?= date('d M H:i', strtotime($m['created_at'])) ?></td>
                                <td class="mono"><?= date('d M H:i', strtotime($m['deleted_at'])) ?></td>
                                <td style="text-align:right">
                                    <form method="post" style="display:inline">
                                        <input type="hidden" name="id" value="<?= (int) $m['id'] ?>">
                                        <button class="btn-ghost btn-sm" type="submit" name="delete_one" value="1">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
